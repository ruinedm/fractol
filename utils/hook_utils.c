...
/*   Updated: 2024/03/19 20:09:42 by mboukour         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	setup_shift(int keycode,t_fractal *fractal)
{
    ...
    scale.imaginary = (fractal->end.imaginary - fractal->start.imaginary) / 800.0;
	if(keycode == UP)
...
int mouse_hook(int mousecode, int x, int y, t_fractal *fractal)
{
    ...
    fractal->start.real = (fractal->start.real - z_mouse.real) * fractal->zoom_factor + z_mouse.real;
	fractal->start.imaginary = (fractal->start.imaginary - z_mouse.imaginary) * fractal->zoom_factor + z_mouse.imaginary;
	fractal->end.real = (fractal->end.real - z_mouse.real) * fractal->zoom_factor + z_mouse.real;
	fractal->end.imaginary = (fractal->end.imaginary - z_mouse.imaginary) * fractal->zoom_factor + z_mouse.imaginary;
	render_fractal(fractal);
    mlx_put_image_to_window(fractal->mlx, fractal->win, fractal->img, 0, 0);
    return (0);
}
